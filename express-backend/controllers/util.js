...
const sameContainerReorder = async (
  { id, order, newOrder },
  whereConfig,
  model
) => {
...
const diffContainerReorder = async (
  { id, s: { sId, order }, d: { dId, newOrder } },
  model
) => {
  const toBeUpdatedSource = updateOrder({
    id: sId,
    order,
    type: "source",
    model,
  });
  const toBeUpdatedTarget = updateOrder({
    id: dId,
    order: newOrder,
    type: "target",
    model,
  ...
  const reattatchAssignees = client.assignee.createMany({
    data: nullAssignees,
  });
...
const badRequest = (res, err) =>
  res
    .status(400)
    .json({
      message: "Whoop! Looks like your http request is missing something.",
      err: err,
    })
    .end();
...
module.exports = {
  sameContainerReorder,
  diffContainerReorder,
  badRequest,
  cookieConfig,
};
